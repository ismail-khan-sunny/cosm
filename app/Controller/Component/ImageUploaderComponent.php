	...
	function uploadImage($config){
		...
		if(move_uploaded_file($files['tmp_name'],WWW_ROOT.$file_path)){
			...
			if(!empty($config['resize'])){
				$this->image = $this->openImage(WWW_ROOT.$file_path);
				$this->width  = imagesx($this->image);
				$this->height = imagesy($this->image);

				// *** 2) Resize image (options: exact, portrait, landscape, auto, crop)
				$resize_image = $this->resizeImage($config['resize']['width'], $config['resize']['height'], 'crop');

				// *** 3) Save image
				$save_image = $this->saveImage(WWW_ROOT.$thumb_path, 100);
