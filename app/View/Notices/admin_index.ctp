	...
	<div class="col-lg-12 col-xs-12 col-md-12 col-sm-12 margin-bottom-10">
		<div class="box-tools pull-right">
			...
			<div class="btn-group">
				...
				<div class="btn-group hidden-nav-xs">
					<?php echo $this->Html->link(__('Add Promotion'), array('action' => 'add'),array('class' => 'btn btn-sm btn-primary')); ?>
				</div>
			</div>
		</div>
	</div>
				...
				<div class="row">
					<div class="col-lg-12 col-xs-12 col-md-12 col-sm-12">
						<p>
					    	...
					    	<?php echo $this->Form->create('Search',array('class'=>'form-inline','url'=>array('controller'=>'Notices','action'=>'admin_index'))); ?>
							...
							<tbody>
								<?php if(!empty($notices)): ?>
									...
									<tr>
										...
										<td>
										<?php 
										$created = strtotime($notice['Notice']['created']);	
										echo h(date("jS F Y", $created));	
										?>&nbsp;</td>
										<td>
											<?php echo $this->Html->link('<i class="fa fa-eye"></i> ', array('action' => 'view', $notice['Notice']['id']),array('class'=>'btn btn-rounded btn-sm btn-icon btn-primary', 'escape'=>false)); ?>
											<?php echo $this->Html->link('<i class="fa fa-edit"></i> ', array('action' => 'edit', $notice['Notice']['id']),array('class'=>'btn btn-rounded btn-sm btn-icon btn-info','escape'=>false)); ?>
											<?php echo $this->Form->postLink('<i class="fa fa-times text-white text"></i>', array('action' => 'delete', $notice['Notice']['id']), array('class'=>'btn btn-rounded btn-sm btn-icon btn-danger', 'escape'=>false), __('Are you sure?')); ?>
										</td>
									</tr>
									<?php $sl++; endforeach; ?>
								<?php else: ?>
