				...
				<div class="row">
					<div class="col-lg-12 col-xs-12 col-md-12 col-sm-12">
						<table class="table table-striped table-bordered dataTable">
							...
							<tbody>
								<?php $sl = $this->Paginator->counter('{:start}'); foreach ($services as $service): ?>
					<tr>
						<td><?php echo h($sl); ?>&nbsp;</td>
						<td><?php echo h($service['Service']['title']); ?>&nbsp;</td>
						<td><?php echo ($service['Service']['description']); ?>&nbsp;</td>
						<td><?php echo $this->Html->image($service['Service']['image'],array('width'=>'80')); ?>&nbsp;</td>
						
						<td><?php echo h($service['Service']['created']); ?>&nbsp;</td>
						<td><?php echo h($service['Service']['modified']); ?>&nbsp;</td>
						<td>
							<?php echo $this->Html->link('<i class="fa fa-eye"></i> ', array('action' => 'view', $service['Service']['id']),array('class'=>'btn btn-rounded btn-sm btn-icon btn-primary', 'escape'=>false)); ?>
							<?php echo $this->Html->link('<i class="fa fa-edit"></i> ', array('action' => 'edit', $service['Service']['id']),array('class'=>'btn btn-rounded btn-sm btn-icon btn-info','escape'=>false)); ?>
							<?php //echo $this->Html->link('<i class="fa fa-times text-white text"></i> ', array('action' => 'delete', $service['Service']['id']),array('class'=>'btn btn-rounded btn-sm btn-icon btn-danger ajax_delete', 'escape'=>false)); ?>
							<?php echo $this->Form->postLink('<i class="fa fa-times text-white text"></i>', array('action' => 'delete', $service['Service']['id']), array('class'=>'btn btn-rounded btn-sm btn-icon btn-danger', 'escape'=>false), __('Are you sure?')); ?>
						</td>
					</tr>
					<?php $sl++; endforeach; ?>
							</tbody>
