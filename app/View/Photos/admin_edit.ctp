					...
					<div class="col-md-12">
						<?php
							if(!empty($this->request->data['Photo']['image'])){
								echo $this->Html->image($this->request->data['Photo']['image']).'<br><br>';
							}
						?>
					</div>
					<?php
						echo $this->Form->hidden('id');
						echo $this->Form->hidden('photo_gallery_id',array('class'=>'form-control'));
						echo $this->Form->hidden('pre_image',array('value'=>$this->request->data['Photo']['image']));
